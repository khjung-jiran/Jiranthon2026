...
final class StorybookService
{
        ...
        foreach (Era::chronological() as $era) {
            ...
            if ($this->isStale($items, $story)) {
                ...
                if (!$draft->isEmpty()) {
                    ...
                    // 실제로 새 스토리가 생성된 경우만 푸시 대상 (기존 스토리가 없었거나 답변 수가 증가한 경우)
                    if ($story === null || \count($items) > (int) $story['response_count']) {
    ...
    private function spawnBackgroundGeneration(string $familyId): void
    {
        ...
        // 잠금 파일이 있고 10분 이내에 생성된 것이면 스킵
        if (\is_file($lockFile)) {
            $mtime = \filemtime($lockFile);

            if ($mtime !== false && (\time() - $mtime) < 600) {
                $this->logger->info("스토리 생성 워커 이미 실행 중 — 스킵: 가족={$familyId}");
        ...
        // 잠금 파일 생성
        \file_put_contents($lockFile, (string) \time());
